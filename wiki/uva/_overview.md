...
The **2026-06-18 final demo check-in** (see [[team-meeting-2026-06-18]]) is the latest ingested artifact: the last scheduled meeting with Sanne before the defence. Laurenz demoed the fully integrated multi-project system (4 account types: UvA member / Bakkie member / combined / guest). Key outcomes: (1) **Vercel deployment decided against** — the team will not deploy to Vercel, prioritising individual thesis work and test cases (closes the long-standing open Vercel gap); (2) **KickstartAI internal-use intent confirmed** — Sanne stated she and another engineer intend to use this work as a starting point for an internal KickstartAI project; (3) **Obsidian graph integration revealed** — Laurenz showed a visual link-map of wiki pages (not previously documented); (4) **Cross-session learning** described by Xiaojing as a distinct wiki capability (summarize project → ingest summary → accessible to team); (5) full re-ingestion + Friday 20.06 final demo planned. See [[_gaps]] for new gaps.

The **2026-06-19 evaluation runplan** (see [[evaluation-runplan-2026-06-19]]) is the prior ingested artifact: the formal experimental design for the headline evaluation experiment — a controlled four-condition comparison (C0/C1/C1r/C2) testing whether the compiled wiki (C2) outperforms top-k retrieved raw chunks (C1r) on correctness (H1) and whether it uses dramatically fewer tokens than a full raw dump (C1, H2). 30 questions across T1 (single-fact), T2 (synthesis), T3 (unanswerable/trap); judge = `claude-sonnet-4-6` (≠ answer model, addressing Sanne's same-model blind-spot concern); build-cost / break-even analysis from `token_usage.md`. The corpus hygiene filter introduces a new entity — **"Westerwoude"** — as a cross-project source to exclude; identity unresolved — see [[_gaps]].
...
Sanne's 2026-06-12 feedback: **"impressed"** the system is this far/functioning; warned it must be **usable by non-technical staff**; recommended Vercel (now decided against, 2026-06-18); flagged **integration into one system** as the biggest challenge; advised picking **~3 evaluation metrics** from literature.
...
- **2026-06-18** — **final demo to Sanne** (fully integrated multi-project system); **Vercel deployment decided against**; **KickstartAI internal-use intent confirmed** (Sanne + another engineer); Obsidian graph integration revealed; cross-session learning described. See [[team-meeting-2026-06-18]].
- **2026-06-19** — **evaluation runplan ingested** (C0/C1/C1r/C2 conditions, H1/H2 hypotheses, T1/T2/T3 question taxonomy, break-even framing); full data re-ingestion planned. See [[evaluation-runplan-2026-06-19]].
- **2026-06-20** — final integration demo (Friday; team + Sanne).
...
- **UI / backend (2026-06-12):** **React/Vite frontend + FastAPI backend**, **markdown-on-disk (no database)**, **git-worktree "Wiki Bot" push** to `main`, SSE-streamed Ingest/Query/Lint operations. **Obsidian graph integration** for visual wiki link maps (revealed 2026-06-18).
...
- **Deployment (2026-06-18 — RESOLVED):** the team **decided against Vercel deployment** to prioritize individual thesis work and test cases. Production auth out of scope. See [[team-meeting-2026-06-18]].
...
- **KickstartAI internal reuse (2026-06-18):** Sanne confirmed she and another engineer intend to use this work as a starting point for an internal KickstartAI project. The strongest concrete realization of the reuse vision to date. See [[_reuse]], [[team-meeting-2026-06-18]].

## Open questions
See [[_gaps]] for the full list. New gaps from the 2026-06-18 demo check-in: **Vercel resolved** (decided against); **Obsidian graph** (new feature, details unspecified); **cross-session learning** (new capability framing, details unspecified); **"another engineer"** at KickstartAI unnamed; **post-defence Bakkie demo** planned but not yet documented; **Friday 20.06 final integration demo** outcome not yet documented; **supervisor full feedback** not yet received.

New gaps from the 2026-06-19 evaluation runplan ingest: **"Westerwoude" project name** (first occurrence, identity unresolved); **specific model versions** (`claude-opus-4-8`, `claude-haiku-4-5`, `claude-sonnet-4-6`) first cited in eval-harness context; **evaluation harness files not ingested** (`questions.json`, `reference-answers-worksheet.md`, `metrics.py`, `pipelines.py`, `run_eval.py`); **`results.json` not yet available**; **judge validation pending**. Resolved: `evaluation/RUN-PLAN.md not ingested` gap is now closed.

Other standing open items: Member 1/Member 2 person-mapping soft signals; multi-project app deployment status; `token_usage.md` not ingested; "Bakkie" identity in multi-project app (real vs. fictional); vector store/embedding model; gap-detector scoring transparency; permission-layer open problems (aggregation/inference leakage, paragraph-label assignment, eval scale, cross-model audit, gpt-5.1 eval environment, restricted pages not yet added); evaluation-metric finalization (~3 metrics; shortlist proposed); new-project use-case experiment design; UI deliverable-status; usability-for-non-technical-staff; un-ingested screenshots/Gantt/GAPS-diagram; un-ingested student-materials corpus & PM Ops folder; recurring check-in cadence; second UvA group; the "Averion" handover; etc.
...
- **KickstartAI main contact:** Sanne Wielinga (Senior ML Engineer). Project handed to her "from Averion" (unidentified — see [[_gaps]]). **Post-thesis internal reuse intent confirmed 2026-06-18.**
