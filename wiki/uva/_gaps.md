...
- **LLM choice / feasibility — NEW, now an active open question.** The first MVP needs to connect to an LLM; "what LLM do they use and is this possible?" is explicitly open — including whether access to the chosen LLM is feasible. Refines the standing tech-stack gap (still no specific LLM/vector store/embedding model/RAG framework chosen). See [[wiki-generation-engine]], [[team-meeting-2026-05-14]].
- **Generator sourcing approach — NEW open design question.** Should the [[generator-module]] generate content **from the wiki** (compiled knowledge base) or **search through the underlying source documents** the wiki is built from? Undecided (Quinten's point). The generator-module page phrasing was softened accordingly. See [[team-meeting-2026-05-14]].
- **Agentic vs. pipeline architecture — NEW open question.** "Should we turn this into agents?" — whether the system is built as an agentic architecture vs. a pipeline is unresolved. See [[wiki-generation-engine]], [[team-meeting-2026-05-14]].
- **Data-understanding scope — NEW open question.** With little data and little preprocessing, what belongs in the CRISP-DM data-understanding phase given the cold-start corpus? Unresolved. See [[team-meeting-2026-05-14]].
- **Admin-per-project governance — NEW (proposal recorded, mechanism unspecified).** The team proposed assigning an **admin per project who governs the wiki page**. How the admin governs, and the relationship to the [[permission-model]] (RBAC/ACL), is unspecified. See [[permission-layer]], [[team-meeting-2026-05-14]].
- **Person↔member mapping — SOFT SIGNAL (still open).** Laurenz holds the GitHub/wiki-architecture MVP — a soft signal toward Laurenz = Member 1 (Ingestion + Wiki Engine); the generator-integration pairing involves Quinten. **Neither is asserted.** The #1–4 owner-by-person mapping remains open. See [[project-team]], [[team-meeting-2026-05-14]].
- **GitHub repo not ingested — NEW re-ingest flag.** Laurenz's GitHub (wiki architecture + MVP code) is referenced but not provided as a source. Flag for future ingest (PRs/repo were always intended as ingestion sources per the kickoff). See [[team-meeting-2026-05-14]], [[ingestion-pipeline]].
...
- **Tech stack — narrowed, still open.** The report names RAG/LLM and generation strategies (direct prompting / RAG / template-driven) and design principles, but **no specific LLM, vector store, embedding model, or RAG framework** is chosen. LLM choice is now an active open question (see 2026-05-14 above). See [[wiki-generation-engine]].
