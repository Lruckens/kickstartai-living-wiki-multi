...
- **Person↔member mapping — soft signal strengthened, STILL OPEN.** Laurenz again holds/owns the GitHub repo and is its KickstartAI-facing contact — strengthening (not proving) Laurenz = Member 1 (Ingestion + Wiki Engine). Not asserted. See [[project-team]], [[user-journeys]].
...
- **Permission model specifics** — Design approach (RBAC? doc-level ACLs?) not yet specified. Design-only deliverable confirmed (MoSCoW: design = Should, fully working = Won't). The mechanism is undecided; data-residency/configurable-backend directions now recorded (see 2026-05-15 above). See [[permission-model]], [[permission-layer]].
