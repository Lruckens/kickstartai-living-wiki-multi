...
- **Kickoff meeting notes not ingested** — Gemini-generated kick-off notes (2026-04-13) and kickoff slides (`20260413 UvA.pdf`, `KickstartAI x UvA - Kick-off.pdf`) are referenced as attachments but their *content* is not ingested; decisions made at kickoff are undocumented. (Note: the `KAI-Intro` deck dated 2026-04-13 is now partially ingested — see the intro-deck section below.)
...
## Surfaced from KickstartAI intro deck ingest (2026-04-13; ingested 2026-06-19)
- **Deck is largely image-only** — `2026-04-13-KAI-Intro.md` is a slide deck whose substantive content (the six-phase adoption model, the partner-ripple diagram, the challenge→solution mappings) is embedded as JPGs with no extractable text. Only slide titles, scattered stat captions, and bullet fragments are legible. Diagram contents are **not ingested** and were **not fabricated**. Flag for OCR/re-ingest. See [[kickstartai]], [[adoption-journey]].
- **Six-phase adoption model contents unknown** — The "six-phase circular adoption model" is *named* in the deck but its actual phases, order, and structure are image-locked and unreadable. [[adoption-journey]] is a contents-pending stub.
- **Kickoff-deck identity unconfirmed** — Whether this `KAI-Intro` deck is specifically `20260413 UvA.pdf` or `KickstartAI x UvA - Kick-off.pdf` (the two decks referenced in the email thread) is **inferred from shared dating, not stated**. The other kickoff deck(s) and the Gemini kickoff notes remain un-ingested. See [[assignment-1-project-definition]].
- **Societal stats are point-in-time / undated-source** — The GDP/Gini/healthcare/climate/congestion figures (~2024 snapshots) come from a pitch deck with no "as-of" provenance beyond the deck date. They will drift and are not authoritative. See [[kickstartai]].
- **Strategy framing vs. pillar vocabulary** — The deck's three strategic aspirations ("be the lighthouse for applied AI" / "mobilize leaders to take AI actions that matter" / "leverage a magnetic AI talent ecosystem") are *adjacent to but not asserted equivalent to* the documented three pillars ("We do/share/connect" ≈ "Apply/Amplify/Activate"). The relationship is unconfirmed; documented as a distinct strategy-deck framing rather than a third equivalent vocabulary.
