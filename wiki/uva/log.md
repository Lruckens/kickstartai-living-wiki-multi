...
**Action:** Ingested the **landmark integrated-MVP showcase + feedback** source. Created [[mockup-artifact-2026-06-12]]. **POSITIVELY CONFIRMED Cara/Meng Cheng = Gap Detector** (6-layer hybrid framework). Documented the **built React/Vite + FastAPI UI** + **git-worktree "Wiki Bot" push architecture**; the generator's Lovable UI + wiki-reader context module + local-laptop deployment problem; the permission layer's email-as-user_id login + project-scoped access. Recorded Sanne's feedback. RESOLVED the generator-sourcing question, the UI-existence gap, and much of the repo-schema gap. Added new gaps.
...
**Notes:** ⚠️ All conflicts handled by status-update + flag, never overwrite history. Total pages now 35.

---

## 2026-06-19 — INGEST
**Source:** 2026-06-15-meeting-notes.md (internal UvA team working meeting notes, development → evaluation phase transition)
**Action:** Created [[team-meeting-2026-06-15]]. Recorded the **fully integrated wiki demo** (Gap Detector dashboard/report page + Permission Layer login landing page wired into one UI), the **git-branch-per-member collaboration workflow** (Laurenz as merge gatekeeper), the concrete **demo/evaluation run-up schedule** (17–22 June; **22.06 final demo + thesis defence** — pinning the previously-unpinned project end date), the **Anthropic API budget / token-cost caution** (pre-parse source docs, esp. PDFs), and the **first concrete evaluation-metric shortlist** (Self-BLEU, BERTScore, LLM-as-Judge: coherence/faithfulness/stakeholder-appropriateness). ADVANCED the evaluation-metric-selection gap; PARTIALLY MITIGATED the laptop-local deployment gap; SUBSTANTIALLY RESOLVED the multi-component-integration gap (demoed).
