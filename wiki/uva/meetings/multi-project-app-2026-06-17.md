...
2. **Clean-slate evaluation corpus** — a brand-new, empty `uva` project subtree can be ingested from scratch with known provenance and per-ingest token counts, giving the evaluation a controlled baseline the long-lived original wiki cannot provide. See [[evaluation-deliverable]], [[evaluation-runplan-2026-06-19]].
...
The `uva` project subtree is the **clean-slate corpus** for the wiki-vs-raw-context experiment: a fresh `.md` ingest builds `wiki/uva/` from `sources/uva/`, token logging records the build cost in `token_usage.md`, and the evaluation runs with `--project uva`. See [[evaluation-runplan-2026-06-19]] for the full runplan (now ingested). This directly supports the 18.06 re-ingest milestone from [[team-meeting-2026-06-15]].
...
## Not ingested
- ~~`evaluation/RUN-PLAN.md`~~ — **NOW INGESTED** as [[evaluation-runplan-2026-06-19]]. Gap resolved.
- `token_usage.md` — per-ingest token logging file; contents not provided. See [[_gaps]].
